package vn.nsn.app.iotp.screen
...
import vn.nsn.app.iotp.screen.transaction.OtpAdvanceActivity
...
public class LoginFragment2 : MvpActivity<CreatePinCodePresenter>(), CreatePinCodeContract, View.OnClickListener {
    ...
    companion object {
        fun newIntent(context: Context, data: String, randomString: String): Intent {
            val notificationIntent = Intent(context, LoginFragment2::class.java)
    ...
    fun getRandomIntent(): String {
        return intent.extras.getString("randomString")
    }
    ...
    fun changeLang(type: String) {
        ...
        val notificationIntent = LoginFragment2.newIntent(applicationContext, getDataIntent(), getRandomIntent())
//        val intent = Intent(this, LoginFragment::class.java)
    ...
    override fun initViews() {
        loadLang()
        tvTittle.text = getString(R.string.app_verify)
        val timeFail = preferenceHelper.getPincodeFail()

        if (timeFail >= 5) {
            var dialog = DialogHelper(this)
            dialog.showAlertDialog("", getString(R.string.locked),
                    Runnable() {
                        finish()
                    })
            ...
            if (diffTime < 300000) {
                ...
                dialog.showAlertDialog("", getString(R.string.lock_5_min),
                        Runnable() {
                            finish()
                        })
...
//            log("the identify is completed")
    ...
    fun onIdentifySuccess() {
        ...
        intent.putExtra("data", getDataIntent())
        intent.putExtra("randomString", getRandomIntent())
        startActivity(intent)
        finish()
