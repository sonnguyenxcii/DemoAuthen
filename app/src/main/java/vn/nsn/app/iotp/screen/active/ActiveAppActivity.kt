package vn.nsn.app.iotp.screen.active
...
//                dialogHelper.showAlertDialog("", ErrorDetails.getErrorString(param!!), object : Runnable {
//                    override fun run() {
//
//                    }
//                })

                dialogHelper.showAlertDialog("", getString(R.string.active_message_failed), object : Runnable {
                    override fun run() {
    ...
    private fun saveAccount(context: Context, bindInfo: BindInfo): Boolean? {
        ...
        try {
            saveResult = fileSystem.saveAccountsToFile(accounts, bindInfo.deviceInfo, Configuration.getInstance(), securityDevice, fileName, context)
        } catch (e: Exception) {
            e.printStackTrace()
            return false
        }
